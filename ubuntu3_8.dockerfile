...
#
#   docker build -f ubuntu3_8.dockerfile -t ubuntu3_8 .
#   docker run -it ubuntu3_8
#   pytest
#
# Or if you prefer a one-liner:
#
#   docker run -it $(docker build -q -f ubuntu3_8.dockerfile .) pytest
#
...
RUN apt update && DEBIAN_FRONTEND=noninteractive apt install -y python3.8 python3.8-dev python3.8-distutils
RUN echo '#!/bin/bash\npython3.8' > /usr/bin/python && \
...
RUN python3.8 setup.py -qqq bdist_wheel -d wheels
