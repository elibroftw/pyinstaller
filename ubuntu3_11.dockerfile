...
#
#   docker build -f ubuntu3_11.dockerfile -t ubuntu3_11 .
#   docker run -it ubuntu3_11
#   pytest
#
# Or if you prefer a one-liner:
#
#   docker run -it $(docker build -q -f ubuntu3_11.dockerfile .) pytest
#
...
RUN apt update && DEBIAN_FRONTEND=noninteractive apt install -y python3.11 python3.11-dev
RUN apt install -y curl
RUN curl -sS https://bootstrap.pypa.io/get-pip.py | python3.11
RUN echo '#!/bin/bash\npython3.11' > /usr/bin/python && \
...
RUN python3.11 setup.py -qqq bdist_wheel -d wheels
...
# Required by tkinter.
RUN apt install -y python3.11-tk
